...
export default function Quantity({
  btnSize, quantity, action, quantityMax, text, inputProps, theme: {backgrounds: {primary, secondary}}
  }: Props){
  ...
  return (
    <div className='flex flex-row items-end w-[80px]'>
      <div className='flex w-min pb-0.5 flex-row items-center'>
        <Button sx={{":hover": {backgroundColor: `rgb(${secondary.rgb})`}, width: btnSize, height: btnSize}} cls={btnCls} 
        disabled={+value > 1 ? false : true} handleClick={() => checkValue(`${+value - 1}`)}>
          <RemoveIcon htmlColor={`rgb(${primary.rgb})`} className='pointer-events-none' 
        ...
        <Button sx={{":hover": {backgroundColor: `rgb(${secondary.rgb})`}, width: btnSize, height: btnSize}} 
        handleClick={() => checkValue(`${+value + 1}`)} 
        cls={btnCls} disabled={+value === quantityMax ? true : false} >
          <AddIcon htmlColor={`rgb(${primary.rgb})`} className='pointer-events-none' sx={{fontSize: btnSize * 1}} />
        </Button>
      </div>
